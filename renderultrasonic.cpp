...
unsigned int gTriggerDigitalSecondOutChannel = 2; //channel to be connected to the module's TRIGGER pin - check the pin diagram in the IDE
unsigned int gEchoDigitalSecondInPin = 3; //channel to be connected to the modules's ECHO pin (digital pin 1) - check the pin diagram in the IDE
...
{
// -------------------------------------------------------------------------------------------------------		
	  // Set the mode of digital pin (1)
    pinMode(context, 0, gEchoDigitalInPin, INPUT); // receiving from ECHO PIN
     pinMode(context, 0, gTriggerDigitalOutChannel, OUTPUT); // 
    pulseIn.init(context, gEchoDigitalInPin, HIGH); //detect HIGH pulses on this pin, init() is called because it's created with the default constructor - to use hasPulsed().
    scope.setup(2, 44100);
    
   
   
      // (2)
    pinMode(context, 0, gEchoDigitalSecondInPin, INPUT); // receiving from ECHO PIN
     pinMode(context, 0, gTriggerDigitalSecondOutChannel, OUTPUT); // 
    pulseIn2.init(context, gEchoDigitalSecondInPin, HIGH); //detect HIGH pulses on this pin
    scope2.setup(2, 44100);
...
// -------------------------------------------------------------------------------------------------------	
    ...
    for(unsigned int n = 0; n<context->digitalFrames; n++){
        gTriggerCount++; // Not sure why it uses the counter
        ...
        if(pulseLength >= gMinPulseLength){ //to avoid spurious readings - so if the pulseLength (in samples) is bigger or equal to gMinPulseLength then go on
            static int count = 0;
            // rescaling according to the datasheet
            distance = duration / gRescale; //time[us] / 58 = distance[cm]
    ...
    for(unsigned int i = 0; i<context->digitalFrames; i++){
        ...
        if(gTriggerCount2 == gTriggerInterval2){
            gTriggerCount2 = 0;
            digitalWriteOnce(context, i, gTriggerDigitalSecondOutChannel, HIGH); //write the status to the trig pin
        } 
        else {
            digitalWriteOnce(context, i, gTriggerDigitalSecondOutChannel, LOW); //write the status to the trig pin
        }
        
        int pulseLength2 = pulseIn2.hasPulsed(context, i); // will return the pulse duration(in samples) if a pulse just ended 
        float duration2 = 1e6 * pulseLength2 / context->digitalSampleRate; // pulse duration in microseconds
        static float distance2 = 0;
        
        if(pulseLength2 >= gMinPulseLength2){
            static int count2 = 0;
            // rescaling according to the datasheet
            distance2 = duration2 / gRescale2; // time[us] / 58 = distance[cm]
            libpd_float("ultra2", distance2);
            if(count2 < 5000){ // we do not want to print the value every time we read it
               rt_printf("distance2: %fcm\n", distance2);
               count2 -= 5000;
            }
            ++count2;
        ...
        // Logging to the scope the pulse inputs (gEchoDigitalInPin) and the distance
        scope.log(digitalRead(context, i, gEchoDigitalSecondInPin), distance2/100);
    }
